...
export default class News extends Component {
  ...
  handleTeslaNews = async() => {
    let url = "https://newsapi.org/v2/everything?q=tesla&from=2023-03-22&sortBy=publishedAt&apiKey=ce1f5de0a39849bb991b5a103008fcda";
    let data = await fetch(url);
    let parsedData = await data.json()
    console.log(data);
    this.setState({articles: parsedData.articles})
  ...
  handleCricketNews = async() => {
    let url = "https://newsapi.org/v2/top-headlines?country=in&apiKey=ce1f5de0a39849bb991b5a103008fcda&q=cri";
    let data = await fetch(url);
    let parsedData = await data.json()
    console.log(data);
    this.setState({articles: parsedData.articles})
  ...
  render() {
    return (
      <div className="container my-3">
        ...
        <button type="button" className="btn btn-outline-info btn-lg ms-3" onClick={this.handleCricketNews}>Cricket News</button>
